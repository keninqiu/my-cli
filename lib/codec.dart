...
bool isDynamicType(String typeName) {
  ...
  if (typeName.endsWith(')') && typeName.startsWith('(')) {
    ...
    for (var i = 0; i < subTypes.length; i++) {
      if (isDynamicType(subTypes[i])) return true;
    }
    return false;
  }
  return false;
...
Uint8List padLeft(Uint8List d, int alignBytes) {
  ...
  if (padLength == alignBytes) padLength = 0;
...
BigInt decodeUint256(Iterable b) {
  return BigInt.parse(hex.encode(List<int>.from(b.take(32).toList())),
      radix: 16);
}
...
Uint8List decodeFixedBytes(Iterable b, int length) {
  return Uint8List.fromList(List<int>.from(b.take(length).toList()));
}

Uint8List decodeBytes(Iterable b) {
  var length = decodeInt(b);
  return Uint8List.fromList(List<int>.from(b.skip(32).take(length).toList()));
}
...
String decodeString(Iterable b) {
  var length = decodeInt(b);
  return String.fromCharCodes(List<int>.from(b.skip(32).take(length)));
}

Uint8List encodeString(String s) {
  var length = encodeInt(s.length);
  return Uint8List.fromList(
      length + padRight(Uint8List.fromList(s.codeUnits), 32));
}

String decodeAddress(Iterable b) {
  return hex.encode(List<int>.from(b.take(32).skip(12).toList()));
}
...
Uint8List encodeFixedLengthList(List<dynamic> l, String type, int length) {
  ...
  if (isDynamicType(type)) {
    ...
    for (var i = 0; i < length; i++) {
      ...
      var encoded = encodeType(type, l[i]);
      if (encoded != null) {
        contents.addAll(encoded);
      }
    }
...
Uint8List? encodeType(String type, dynamic data) {
  ...
  if (type.startsWith('(') && type.endsWith(')')) {
    ...
    if (subtypes.length != (data as List).length) {
      throw Exception(
          "incompatibal input length and contract abi arguments for ${type}");
    }
    ...
    for (var i = 0; i < subtypes.length; i++) {
      if (isDynamicType(subtypes[i])) {
        ...
        var encoded = encodeType(subtypes[i], data[i]);
        if (encoded != null) {
          contents.addAll(encoded);
        }
      } else {
