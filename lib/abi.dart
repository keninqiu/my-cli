...
class ContractInput {
  ...
  String get type {
    ...
    return _type.replaceFirst(
        'tuple', '(' + components.map((i) => i.type).join(',') + ')');
  }
      ...
      : name = json['name'],
        ...
        components = List<ContractInput>.from(
            ((json['components'] ?? []) as List)
                .map((i) => ContractInput.fromJson(i)));
}
...
class ContractABIEntry {
  ...
  ContractABIEntry.fromJson(Map<String, dynamic> json)
      : name = json['name'],
        ...
        inputs = List<ContractInput>.from(
            json['inputs'].map((i) => ContractInput.fromJson(i))),
        outputs = List<ContractOutput>.from(
            json['outputs'].map((i) => ContractOutput.fromJson(i)));
  ...
  Uint8List composeCall(Map<String, dynamic> callParams) {
    var encoded = encodeType(
        paramDescription, inputs.map((n) => callParams[n.name]).toList());
    if (encoded != null) {
      return Uint8List.fromList(methodBytes + encoded);
    }
    return new Uint8List(10);
  }
...
class ContractABI {
  ...
  Map<String?, ContractABIEntry?> methodIdMap =
      new Map(); // maps from method id to method entry
  Map<String?, ContractABIEntry?> methodNameMap =
      new Map(); // maps from method name to method entry

  ContractABI.fromJson(List<dynamic> json)
      : abis = List<ContractABIEntry>.from(json
            .where((i) =>
                i['type'] ==
                'function') // only processes function abi, ignores events and constructor
  ...
  ContractCall decomposeCall(Uint8List data) {
    ...
    if (abiEntry == null) {
      throw Exception(
          "Method id ${methodId} not found in abi, check whether input and abi matches");
    }
